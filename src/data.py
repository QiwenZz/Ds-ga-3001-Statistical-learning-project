...
def get_dataloaders(path, args):
    ...
    if args['segmentation']: 
        X, y, idx_to_class = import_processed_data(args, f"data/processed_{args['size']}_seg")
    else: 
    ...
    if args['segmentation']: 
        transform_lst = transform_lst[:4]
        val_transform = None
    elif args['random']: 
        transform_lst = [transforms.ToPILImage(),transforms.RandAugment(), transforms.ToTensor()] + transform_lst
    elif args['auto']: 
        ...
        transform_lst = [transforms.ToPILImage(), cifar, imagenet, cvhm, transforms.ToTensor()] + transform_lst
    else: 
        transform_lst.append(transforms.Normalize(mean=args['norm_mean'], std=args['norm_std']))
